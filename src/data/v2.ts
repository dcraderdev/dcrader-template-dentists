export const practice = {
  name: "Elara Smile Studio",
  tagline: "Where artistry meets dentistry",
  phone: "(310) 555-0247",
  email: "concierge@elarasmilestudio.com",
  address: "9200 Wilshire Blvd, Suite 800, Beverly Hills, CA 90212",
  founded: "2011",
  hero: {
    headline: "Your Signature\nSmile Awaits",
    subheadline: "Bespoke cosmetic dentistry crafted for those who refuse to compromise on perfection.",
    ctaText: "Request a Consultation",
    image: "/images/photo-1620916566398-39f1143ab7be-hero.webp",
  },
  hours: [
    { day: "Monday–Friday", time: "9:00 AM – 6:00 PM" },
    { day: "Saturday", time: "10:00 AM – 4:00 PM" },
    { day: "Sunday", time: "By appointment" },
  ],
  services: [
    { name: "Porcelain Veneers", desc: "Ultra-thin, hand-crafted veneers for a flawless, luminous smile.", icon: "✦" },
    { name: "Smile Makeovers", desc: "Comprehensive full-mouth transformation designed around your unique facial aesthetics.", icon: "◈" },
    { name: "Teeth Whitening", desc: "Boutique-grade Zoom! whitening with custom shade mapping for brilliant, lasting results.", icon: "◇" },
    { name: "Dental Implants", desc: "Titanium-root implants with premium zirconia crowns indistinguishable from natural teeth.", icon: "⬡" },
    { name: "Invisalign Platinum", desc: "Clear aligner therapy from a certified Diamond Invisalign provider.", icon: "◉" },
    { name: "Gum Contouring", desc: "Precision soft-tissue sculpting to perfect your gumline and frame your teeth beautifully.", icon: "⬟" },
    { name: "Composite Bonding", desc: "Artisan-applied composite to subtly reshape and enhance your natural smile.", icon: "◈" },
    { name: "Maintenance Program", desc: "VIP quarterly care program with priority scheduling and concierge service.", icon: "✦" },
  ],
  insurance: [
    "PPO plans accepted", "Financing via CareCredit", "LendingClub Patient Solutions",
    "HSA/FSA accepted", "Complimentary benefit verification",
  ],
  team: [
    {
      name: "Dr. Isabelle Fontaine",
      role: "Cosmetic & Aesthetic Dentist",
      bio: "Trained at NYU College of Dentistry and the Kois Center. Featured in Vogue, Harper's Bazaar, and New Beauty. Recognized as one of LA's top cosmetic dentists for 8 consecutive years.",
      image: "/images/photo-1614608682850-e0d6ed316d47-thumb.webp",
    },
    {
      name: "Dr. Ethan Ashby",
      role: "Prosthodontist",
      bio: "Fellowship-trained specialist in complex full-mouth restorations and implant-supported prosthetics. Brings surgical precision to every case.",
      image: "/images/photo-1560250097-0b93528c311a-thumb.webp",
    },
    {
      name: "Camille Laurent",
      role: "Smile Design Consultant",
      bio: "Digital smile design specialist using DSD Pro software to co-create your ideal smile before a single tooth is prepared.",
      image: "/images/photo-1487412720507-e7ab37603c6f-thumb.webp",
    },
    {
      name: "André Voss",
      role: "Lead Ceramist (In-House Lab)",
      bio: "Master ceramist with 20+ years crafting bespoke restorations. Each porcelain veneer is a hand-layered work of art.",
      image: "/images/photo-1472099645785-5658abf4ff4e-thumb.webp",
    },
  ],
  reviews: [
    { name: "Victoria S.", stars: 5, text: "I flew in from New York specifically to see Dr. Fontaine. My porcelain veneers are the most beautiful thing I've ever seen — and they look completely natural. Worth every penny.", date: "April 2026" },
    { name: "Marcus P.", stars: 5, text: "The DSD process was incredible. I saw exactly what my smile would look like before they did anything. The result was even better than the preview.", date: "March 2026" },
    { name: "Sophia R.", stars: 5, text: "Elara is more like a luxury spa than a dentist office. From the champagne in the lounge to the impeccable work Dr. Fontaine does — it's a genuinely extraordinary experience.", date: "February 2026" },
    { name: "James A.", stars: 5, text: "I had severe dental anxiety and they made every step feel safe and luxurious. Dr. Ashby rebuilt my entire smile and I couldn't be happier.", date: "January 2026" },
  ],
  beforeAfter: [
    { before: "/images/photo-1629909615184-74f495363b67-card.webp", after: "/images/photo-1620916566398-39f1143ab7be-card.webp", label: "Full Porcelain Veneer Set" },
    { before: "/images/photo-1609840114035-3c981b782dfe-card.webp", after: "/images/photo-1606811841689-23dfddce3e95-card.webp", label: "Smile Makeover — 18 Teeth" },
  ],
  officePhotos: [
    { url: "/images/photo-1631217868264-e5b90bb7e133-card.webp", caption: "Private consultation lounge" },
    { url: "/images/photo-1629909615184-74f495363b67-card.webp", caption: "Bespoke treatment suite" },
    { url: "/images/photo-1606811841689-23dfddce3e95-card.webp", caption: "In-house ceramics studio" },
  ],
};
